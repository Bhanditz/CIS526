...
object GizaAlign{
    ...
    var dbPathToken:String = FileManager.getResource("resources/giza/hi-en.aligns")
    ...
    def isAligned(name:String, i:Int, j:Int): Float = {
        ...
        if (j == a.getOrElse(i, -1)) {
         println("returning true: "+i+" "+j+" "+a.getOrElse(i, -1))
         return 1 
        }
        println("returning false : "+i+" "+j+" "+a.getOrElse(i, -1))
        return 0
    }
